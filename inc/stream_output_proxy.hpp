...
std::string to_string_symmetric(const error_type& x);
std::string to_latex_string_symmetric(const error_type& x);

std::string to_string_pdg(const error_type& x);
std::string to_latex_string_pdg(const error_type& x);
...
namespace numerical_output
{
  ...
  struct error_output_proxy
  {
    ...
    {
      return q.m_os << to_string_symmetric(right_hand);
    }
  ...
  //std::cout << numerical_output::emoc << a;
	...
	struct latex_error_output_proxy
	{
		...
		friend std::ostream & operator<<(latex_error_output_proxy const& q,
		                                 error_type const& right_hand)
		{
      return q.m_os << to_latex_string_symmetric(right_hand);
		}
                                    ...
                                    std::tuple<std::string,error_type> const& right_hand)
    {
      ...
      return q.m_os << "\\def\\"
                    ...
                    << to_latex_string_symmetric(std::get<1>(right_hand))
                    << "}";
    }
